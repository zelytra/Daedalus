  ...
  public boolean onCommand(
    ...
    if (!Bukkit.getServer().getPluginManager().isPluginEnabled("WorldEdit")) {
      player.sendMessage(
          Message.getPlayerPrefixe()
              + "§cWorldEdit is needed on the server to perform this command.");
      return false;
    }
    if (args.length == 1 && args[0].equalsIgnoreCase("help")) {
      ...
      if (WEH.saveStructure()) {
        player.sendMessage(
            Message.getPlayerPrefixe()
                + "§aStructure file saved :§6 "
                + WEH.getStructureName()
                + ".struct");
        return true;
      } else {
      ...
      player.sendMessage(
          Message.getPlayerPrefixe()
              + "§cWrong syntax please refer to §6/structure help §ccommand.");
      return false;
    }
  }
}
