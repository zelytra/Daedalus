...
public class StructureCommands implements CommandExecutor {
	...
	public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String msg,
		...
		if (!Bukkit.getServer().getPluginManager().isPluginEnabled("WorldEdit")) {
			player.sendMessage(
					Message.getPlayerPrefixe() + "§cWorldEdit is needed on the server to perform this command.");
			return false;
		}
		if (args.length == 1 && args[0].equalsIgnoreCase("help")) {
			...
			if (region != null) {
				...
				if (((region.getLength() + 1) / 8.0) % 2 == 0 || ((region.getWidth() + 1) / 8.0) % 2 == 0) {
					player.sendMessage(Message.getPlayerPrefixe() + "§cWrong structure size :§6 " + region.getWidth()
							+ " / " + region.getLength());
					return false;
				}
			...
			if (WEH.saveStructure()) {
				player.sendMessage(Message.getPlayerPrefixe() + "§aStructure file saved :§6 " + WEH.getStructureName()
						+ ".struct");
				return true;
			} else {
			...
			player.sendMessage(
					Message.getPlayerPrefixe() + "§cWrong syntax please refer to §6/structure help §ccommand.");
			return false;
		}
	}
}
