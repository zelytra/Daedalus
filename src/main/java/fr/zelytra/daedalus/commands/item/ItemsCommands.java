  ...
  public boolean onCommand(
    ...
    if (args.length == 1 && args[0].equalsIgnoreCase("help")) {
      ...
      if (CustomMaterial.getByName(args[0]) != null) {
        player
            .getInventory()
            .addItem(new CustomItemStack(CustomMaterial.getByName(args[0]), 1).getItem());
        player.sendMessage(
            Message.getPlayerPrefixe()
                + GameSettings.LANG.textOf("command.giveItemPrefix")
                + 1
                + " §6§l"
                + args[0]
                + GameSettings.LANG.textOf("command.giveItemSuffix")
                + player.getName());
        playItemSound(player);
        return true;
      } else {
        player.sendMessage(
            Message.getPlayerPrefixe() + GameSettings.LANG.textOf("command.itemExist"));
        return false;
      }
      ...
      if (CustomMaterial.getByName(args[0]) != null) {
        CustomItemStack cItem =
            new CustomItemStack(CustomMaterial.getByName(args[0]), Integer.parseInt(args[1]));
