				...
				for (Map.Entry<Character, RecipeChoice> map : this.shapedRecipe.getChoiceMap().entrySet()) {
					if (map.getValue() != null) {
						if (CustomItemStack.hasTag(map.getValue().getItemStack())) {
							RecipeChoice.ExactChoice item = new RecipeChoice.ExactChoice(new CustomItemStack(
									CustomItemStack.getCustomMaterial(map.getValue().getItemStack()), lang).getItem());
