      ...
      for (String loot : yml.getConfigurationSection(tableName + ".loots").getKeys(false)) {
          ...
          if (material != Material.ENCHANTED_BOOK) {
            ...
            meta.addEnchant(
                Enchantment.getByName(
                    yml.getString(tableName + ".loots." + loot + ".enchant.type").toUpperCase()),
                yml.getInt(yml.getString(tableName + ".loots." + loot + ".enchant.level")),
                true);
            item.setItemMeta(meta);
          } else {
            item =
                Utils.BookEnchantedItemStack(
                    material,
                    Enchantment.getByName(
                        yml.getString(tableName + ".loots." + loot + ".enchant.type")
                            .toUpperCase()),
        ...
        if (yml.getString(tableName + ".loots." + loot + ".effect.type") != null) {
          ...
          if (material == Material.POTION || material == Material.SPLASH_POTION) {
            ...
            meta.displayName(
                Component.text()
                    .content("Potion of " + potionEffectType.getName().toLowerCase())
                    .build());
            item.setItemMeta(meta);

          } else {
            System.out.println(
                "LootParser problem : Cannot add potion effect on "
                    + yml.getString(tableName + ".loots." + loot)
                    + " item type");
          }
        }
