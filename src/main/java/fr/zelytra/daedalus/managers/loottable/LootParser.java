		...
		for (String tableName : yml.getKeys(false)) {
			...
			for (String loot : yml.getConfigurationSection(tableName + ".loots").getKeys(false)) {
				Material material = Material
						.getMaterial(yml.getString(tableName + ".loots." + loot + ".material").toUpperCase());
				if (material == null) {
					System.out.println("LootParser problem with : "
							+ yml.getString(tableName + ".loots." + loot + ".material").toUpperCase());
					continue;
				}
				...
				if (yml.getString(tableName + ".loots." + loot + ".enchant.type") != null) {
					if (Enchantment.getByName(
							yml.getString(tableName + ".loots." + loot + ".enchant.type").toUpperCase()) == null) {
						System.out.println("LootParser problem with : "
								+ yml.getString(tableName + ".loots." + loot + ".enchant.type").toUpperCase());
						continue;
					}
					if (material != Material.ENCHANTED_BOOK) {
						...
						meta.addEnchant(
								Enchantment.getByName(
										yml.getString(tableName + ".loots." + loot + ".enchant.type").toUpperCase()),
								yml.getInt(yml.getString(tableName + ".loots." + loot + ".enchant.level")), true);
						item.setItemMeta(meta);
					} else {
						item = Utils.BookEnchantedItemStack(material,
								Enchantment.getByName(
										yml.getString(tableName + ".loots." + loot + ".enchant.type").toUpperCase()),
								yml.getInt(tableName + ".loots." + loot + ".enchant.level"));
					}
				}

				/* Potion reader */
				if (yml.getString(tableName + ".loots." + loot + ".effect.type") != null) {
					if (PotionEffectType.getByName(
							yml.getString(tableName + ".loots." + loot + ".effect.type").toUpperCase()) == null) {
						System.out.println("LootParser problem with : "
								+ yml.getString(tableName + ".loots." + loot + ".effect.type").toUpperCase());
						continue;
					}
					if (material == Material.POTION || material == Material.SPLASH_POTION) {
						...
						PotionEffectType potionEffectType = PotionEffectType
								.getByName(yml.getString(tableName + ".loots." + loot + ".effect.type").toUpperCase());
						...
						potionMeta.setColor(Color.fromRGB(ThreadLocalRandom.current().nextInt(0, 255 + 1),
						...
						meta.displayName(Component.text()
								.content("Potion of " + potionEffectType.getName().toLowerCase()).build());
						item.setItemMeta(meta);

					} else {
						System.out.println("LootParser problem : Cannot add potion effect on "
								+ yml.getString(tableName + ".loots." + loot) + " item type");
					}
				}
