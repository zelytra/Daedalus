...
public enum LayerEnum {
  ...
  public static LayerEnum getByLevel(int y) {
    for (LayerEnum layerEnum : LayerEnum.values()) if (layerEnum.getY() == y) return layerEnum;
  ...
  public BlockType getBlockTypes() {
    if (Math.random() <= this.luck) return this.blockTypes.get(0);
    else return this.blockTypes.get(1);
  }
}
