...
public class MazeHandler {
	 ...
	 *            Size of the maze
	 * @param complexity
	 *            True if complex maze
	 * @param land
	 *            List of structure to generate
	 */
		...
		Bukkit.getScheduler().runTask(Daedalus.getInstance(), () -> {
			...
			for (int x = 0; x < this.maze.getSize(); x++) {
				for (int z = 0; z < this.maze.getSize(); z++) {
					...
					logPlayers("§6§l" + GameSettings.LANG.textOf("maze.generateBlock") + " [§e" + progress + "%§6]");
				}
			}
	...
	public void demoGenerateScaleMaze() {
		Location origin = getOrigin(((this.maze.getSize() * this.maze.getScale() - 1 * this.maze.getScale()) / 2)
				+ ((this.maze.getSize() - 1) / 2) + 1);
		this.maze.setOrigin(origin);
		...
		Bukkit.getScheduler().runTask(Daedalus.getInstance(), () -> {
			...
			for (int x = 0; x < grid.length; x++) {
				for (int z = 0; z < grid.length; z++) {
					...
					logPlayers(GameSettings.LANG.textOf("§6§l" + "maze.generateBlock") + " [§e" + progress + "%§6]");
				}
			}
			WEH.getEditSession().close();
		});
	...
	public void generateScaleMaze() {
		...
		Bukkit.getScheduler().runTask(Daedalus.getInstance(), () -> {
			...
			for (int x = 0; x < grid.length; x++) {
				for (int z = 0; z < grid.length; z++) {
					...
					if ((System.currentTimeMillis() - timer) % 100 == 0) {
						int progress = (int) (count * 100 / (Math.pow(grid.length, 2)));
						logPlayers(
								"§6§l" + GameSettings.LANG.textOf("maze.generateBlock") + " [§e" + progress + "%§6]");
					}
				}
			}
			WEH.getEditSession().close();
			...
			Bukkit.broadcastMessage(
					Message.getPlayerPrefixe() + "§8" + GameSettings.LANG.textOf("maze.generateStructures"));
			count = 0;
			for (Map.Entry<BoundingBox, Structure> entry : Daedalus.getInstance().getStructureManager()
				...
				int progress = ((count * 100)
						/ Daedalus.getInstance().getStructureManager().getStructuresPosition().size());
				logPlayers("§6§l" + GameSettings.LANG.textOf("maze.generateStructures") + " §8[§f" + progress + "%§8]");
				count++;
			}
			...
			for (Map.Entry<BoundingBox, Structure> entry : Daedalus.getInstance().getStructureManager()
				...
				LootTable lootTable = Daedalus.getInstance().getStructureManager().getLootTableManager()
				...
				int progress = ((count * 100)
						/ Daedalus.getInstance().getStructureManager().getStructuresPosition().entrySet().size());
				logPlayers("§6§l" + GameSettings.LANG.textOf("maze.generateLoots") + " §8[§f" + progress + "%§8]");
			}
			/* Locating boss spawn area */
			Bukkit.broadcastMessage(
					Message.getPlayerPrefixe() + "§8" + GameSettings.LANG.textOf("maze.locatingBosses"));
			count = 0;
			for (Map.Entry<BoundingBox, Structure> entry : Daedalus.getInstance().getStructureManager()
					.getStructuresPosition().entrySet()) {
				if (entry.getValue().getType() == StructureType.TEMPLE) {
					...
					logPlayers("§6§l" + GameSettings.LANG.textOf("maze.locatingBosses") + " §8[§f" + progress + "%§8]");
				}
			}
			...
			for (int r = radius; r > 35; r--) {
				...
				if ((System.currentTimeMillis() - timer) % 100 == 0) {
					int progress = (int) (count * 100 / (Math.pow(grid.length, 2)));
					logPlayers("§6§l" + GameSettings.LANG.textOf("maze.shrinkageArea") + " §8[§f" + progress + "%§8]");
				}
			...
			if (timeInMin <= 0)
				Bukkit.broadcastMessage(Message.getPlayerPrefixe() + GameSettings.LANG.textOf("maze.generationData")
						+ timeInSec + "s§8]");
			else
				Bukkit.broadcastMessage(Message.getPlayerPrefixe() + GameSettings.LANG.textOf("maze.generationData")
						+ timeInMin + "m" + timeInSec + "s§8]");
		});
	}
	...
	private Location getOrigin(int gridSize) {
		return new Location(center.getWorld(), center.getX() - (gridSize / 2.0) + 1, center.getY(),
				center.getZ() - (gridSize / 2.0) + 1);
	}

	private void addShrinkPos(Vector2 v, Location origin) {
		if (v.x == 0 && v.z == 0) {
			Daedalus.getInstance().getStructureManager().getShrinkManager().getWorkloadThread()
