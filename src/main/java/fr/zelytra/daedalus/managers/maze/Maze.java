...
public class Maze {
	...
	private void generateGrid() {
		...
		for (Structure area : land) {
			...
			int width = (area.getRegion().getWidth() + 1) / (this.scale + 1)
					+ ((area.getRegion().getWidth() + 1) / (this.scale + 1)) - 1;
			int length = (area.getRegion().getLength() + 1) / (this.scale + 1)
					+ ((area.getRegion().getLength() + 1) / (this.scale + 1)) - 1;
			int originX;
			int originZ;
			if (area.getType() == StructureType.BASE) {
				...
				for (Structure surroundedStruct : Daedalus.getInstance().getStructureManager().structureSurrounded
						.getStructures()) {
					int sWidth = (surroundedStruct.getRegion().getWidth() + 1) / (this.scale + 1)
							+ ((surroundedStruct.getRegion().getWidth() + 1) / (this.scale + 1)) - 1;
					int sLength = (surroundedStruct.getRegion().getLength() + 1) / (this.scale + 1)
							+ ((surroundedStruct.getRegion().getLength() + 1) / (this.scale + 1)) - 1;
					Vector2 coordinate = getRandomStructurePosition(sWidth, sLength, this.spacing,
				...
				Vector2 coordinate = getRandomStructurePosition(width, length, this.spacing,
						new Vector2(this.size / 2, this.size / 2), this.size);
				if (coordinate == null) {
					continue;
				}
	...
	private void generateWay() {
		...
		while (idx > 0) {
			...
			if ((System.currentTimeMillis() - timer) % 500 == 0) {
				progress = (int) ((((Math.pow(this.size - 1, 2) / 4.0) - progress) * 100)
		...
		Bukkit.broadcastMessage(Message.getPlayerPrefixe() + GameSettings.LANG.textOf("maze.matrixTime")
				+ (System.currentTimeMillis() - time) + "ms§8]");
	}
	...
	private void generateStructureGrid(Vector2 coordinate, Structure area, int width, int length) {
		...
		BoundingBox areaBox = new BoundingBox(
				Math.floor(((coordinate.x / 2) * (this.scale + 1) + coordinate.x % 2) + this.origin.getX()), 0,
				Math.floor(((coordinate.z / 2) * (this.scale + 1) + coordinate.z % 2) + this.origin.getZ()),
				Math.floor((((coordinate.x / 2) * (this.scale + 1) + coordinate.x % 2) + area.getRegion().getWidth())
						+ this.origin.getX()),
				255,
				Math.floor(((coordinate.z / 2 * (this.scale + 1) + coordinate.z % 2) + area.getRegion().getLength())
						+ this.origin.getZ()));
