...
public class MessageManager {
      ...
      case TEAM:
        ...
        for (Player player :
            Daedalus.getInstance()
                ...
                .getFactionOf(FactionsEnum.SPECTATOR)
                .getPlayerList())
          if (player.isOp()) Objects.requireNonNull(player).sendMessage(getFormattedMessage());
  ...
  public String getFormattedMessage() {
    switch (channel) {
      case GLOBAL:
        {
          return ChatColor.of("#808080")
              + GameSettings.LANG.textOf("chat.global")
              + senderTeam.getType().getPrefix()
              + sender.getName()
              + "§7 > §f"
              + message.substring(1);
        }
      case TEAM:
        {
          return ChatColor.of("#808080")
              + GameSettings.LANG.textOf("chat.team")
              + senderTeam.getType().getPrefix()
              + sender.getName()
              + "§7 > §f"
              + message;
      ...
      case SPECTATOR:
        {
          if (!senderTeam.isAlive(sender))
            return ChatColor.of("#808080")
                + GameSettings.LANG.textOf("chat.spec")
                + senderTeam.getType().getPrefix()
                + senderTeam.getType().getPrefix()
                + sender.getName()
                + "§7 > §f"
