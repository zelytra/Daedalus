...
public class Cooldown {
  ...
  public static boolean cooldownCheck(Player player, String tag) {
    ...
    for (Map.Entry<Cooldown, Player> entry : cooldownsList.entrySet()) {
      if (entry.getKey().getTag().equalsIgnoreCase(tag)
          && entry.getValue().getUniqueId() == player.getUniqueId()) {
        toRemove = entry.getKey();
        if (!toRemove.isFinished()) {
          player.sendMessage(
              Message.getPlayerPrefixe() + GameSettings.LANG.textOf("item.coolDown") + toRemove);
          return false;
        }
      }
    }
