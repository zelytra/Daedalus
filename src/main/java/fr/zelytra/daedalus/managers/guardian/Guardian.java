...
public class Guardian implements Listener {
  ...
  private static final NamespacedKey spawnLocKey =
      new NamespacedKey(Daedalus.getInstance(), "spawnLoc");

  @Getter private final int health = 100;
  ...
  private LivingEntity initGuardian() {
            ...
            .spawn(
                ...
                entity -> {
                  ...
                  entity
                      .getEquipment()
                      .setHelmet(
                          Utils.EnchantedItemStack(
                              Material.NETHERITE_HELMET, Enchantment.PROTECTION, 2));
                  entity
                      .getEquipment()
                      .setChestplate(
                          Utils.EnchantedItemStack(
                              Material.NETHERITE_CHESTPLATE, Enchantment.PROTECTION, 3));
                  entity
                      .getEquipment()
                      .setLeggings(
                          Utils.EnchantedItemStack(
                              Material.NETHERITE_LEGGINGS, Enchantment.PROTECTION, 2));
                  entity
                      .getEquipment()
                      .setBoots(
                          Utils.EnchantedItemStack(
                              Material.NETHERITE_BOOTS, Enchantment.PROTECTION, 2));
                  entity
                      .getEquipment()
                      .setItemInMainHand(
                          Utils.EnchantedItemStack(
                              Material.NETHERITE_AXE, Enchantment.SHARPNESS, 5));
                  entity.addPotionEffect(
                      new PotionEffect(PotionEffectType.SPEED, 999999999, 1, false, false));
                });
    ...
    dataContainer.set(
        ...
        this.spawnLoc.getWorld()
            + ","
            + (int) this.spawnLoc.getX()
            + ","
            + (int) this.spawnLoc.getY()
            + ","
            + (int) this.spawnLoc.getZ());
        ...
        Spider spider =
            (Spider)
                location
                    .getWorld()
                    .spawnEntity(
                        location, EntityType.SPIDER, CreatureSpawnEvent.SpawnReason.CUSTOM);
  ...
  private void summonSkeleton() {
    ...
    if (isPlayer) {
      ...
      for (int x = 1; x <= 4; x++) {
                ...
                location
                    ...
                    .spawnEntity(
                        ...
                        CreatureSpawnEvent.SpawnReason
                            .CUSTOM); // The spawned entity is a LivingEntity

        // Create and enchant the bow
        ItemStack bow =
            Utils.EnchantedItemStack(
                Material.BOW, Enchantment.PUNCH, 2); // Custom method for creating enchanted items
        ...
        entity
            .getEquipment()
            .setItemInMainHandDropChance(0); // Prevent the bow from being dropped on death

        // Create and set the enchanted helmet
        ItemStack helmet =
            Utils.EnchantedItemStack(Material.NETHERITE_HELMET, Enchantment.PROTECTION, 4);
        entity.getEquipment().setHelmet(helmet);
        entity
            .getEquipment()
            .setHelmetDropChance(0); // Prevent the helmet from being dropped on death
    ...
    for (Entity e : nearbyEntities) {
      if (e instanceof Player && ((Player) e).getGameMode() == GameMode.SURVIVAL) {
        ...
        player.addPotionEffect(
            new PotionEffect(PotionEffectType.BLINDNESS, 100, 1, false, false, true));
        player.addPotionEffect(
            new PotionEffect(PotionEffectType.SLOWNESS, 100, 1, false, false, true));
      }
    }
  }
  ...
  private void ejection() {
    ...
    if (isPlayer) {
      ...
      for (Entity e : nearbyEntities) {
        ...
        if (e instanceof Player && ((Player) e).getGameMode() != GameMode.SURVIVAL) continue;

        Vector delta =
            new Vector(
                e.getLocation().getX() - entity.getLocation().getX(),
                0,
                e.getLocation().getZ() - entity.getLocation().getZ());
        double norme =
            Math.sqrt(
                Math.pow(delta.getX() + 0.01, 2)
                    + Math.pow(delta.getY() + 0.01, 2)
                    + Math.pow(delta.getZ() + 0.01, 2));
        final int coef = 2;

        Vector dir =
            new Vector(
                (delta.getX() / norme) * coef,
                (delta.getY() / norme) + 1,
                  ...
                  for (int d = 0; d <= 20; d += 1) {
                    ...
                    Bukkit.getScheduler()
                        .runTask(
                            ...
                            () -> {
                              Entity dragonBall =
                                  entity.getWorld().spawnEntity(ballLoc, EntityType.FIREBALL);
                      ...
                      .runTask(
                          ...
                          () ->
                              entity
                                  .getLocation()
                                  .getWorld()
                                  .spawnEntity(fangLoc, EntityType.EVOKER_FANGS));
                  size += 0.3;
                  try {
                    Thread.sleep(100);
                  } catch (InterruptedException e) {
