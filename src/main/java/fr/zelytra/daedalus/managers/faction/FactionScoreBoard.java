...
public class FactionScoreBoard {
	...
	public void update(TimeManager timeManager) {
		for (Player player : faction.getPlayerList()) {
			if (player.getScoreboard().getObjective(faction.getType().getName())
					.getDisplaySlot() != DisplaySlot.SIDEBAR)
				objective.setDisplaySlot(DisplaySlot.SIDEBAR);
		}

		scoreboard.getTeam("episode")
		...
		scoreboard.getTeam("border")
				.setPrefix(GameSettings.LANG.textOf("scoreboard.border")
						+ (Daedalus.getInstance().getStructureManager().getShrinkManager().getWorkloadThread() == null
								? "§cNull"
								: Daedalus.getInstance().getStructureManager().getShrinkManager().getBorderRadius()));
		scoreboard.getTeam("stateMino")
				.setPrefix(GameSettings.LANG.textOf("scoreboard.stateMino") + (DeathListener.hasMinoSpawn
						? (isMinotaur()
								? GameSettings.LANG.textOf("scoreboard.stateMinoAlive")
								: GameSettings.LANG.textOf("scoreboard.stateMinoDead"))
						: "§6§k4269"));
		scoreboard.getTeam("alive").setPrefix(
				GameSettings.LANG.textOf("scoreboard.alive") + getAlivePlayer() + "§6/§a" + getTotalPlayer());
	...
	private void registerAllTeams() {
		for (FactionsEnum factionsEnum : FactionsEnum.values()) {
			...
			team.setOption(org.bukkit.scoreboard.Team.Option.DEATH_MESSAGE_VISIBILITY,
					org.bukkit.scoreboard.Team.OptionStatus.NEVER);
