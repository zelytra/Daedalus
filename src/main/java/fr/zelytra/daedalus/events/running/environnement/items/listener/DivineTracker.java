...
public class DivineTracker implements Listener {
	...
	public void interactEvent(CustomItemUseEvent e) {
		...
		if (e.getMaterial() != CustomMaterial.DIVINE_TRACKER)
			return;
		...
		if (player.isSneaking()) {
			...
			if (!item.getItemMeta().getPersistentDataContainer().has(templeKey, PersistentDataType.INTEGER)) {
				...
				if (isSummon(temples.get((temples.keySet().toArray())[0])) == null)
					printStructureDistance(player, (BoundingBox) (temples.keySet().toArray())[0],
							temples.get((temples.keySet().toArray())[0]));
				else
					printGodDistance(player, isSummon(temples.get((temples.keySet().toArray())[0])),
							temples.get((temples.keySet().toArray())[0]).getGod());

			} else {

				int value = item.getItemMeta().getPersistentDataContainer().get(templeKey, PersistentDataType.INTEGER)
						+ 1;
				...
				if (value >= temples.size()) {
					...
					if (isSummon(temples.get((temples.keySet().toArray())[value])) == null)
						printStructureDistance(player, (BoundingBox) (temples.keySet().toArray())[value],
								temples.get((temples.keySet().toArray())[value]));
					else
						printGodDistance(player, isSummon(temples.get((temples.keySet().toArray())[value])),
			...
			if (!item.getItemMeta().getPersistentDataContainer().has(templeKey, PersistentDataType.INTEGER)) {
				...
				if (isSummon(temples.get((temples.keySet().toArray())[item.getItemMeta().getPersistentDataContainer()
						.get(templeKey, PersistentDataType.INTEGER)])) == null)
					printStructureDistance(player,
							(BoundingBox) (temples.keySet().toArray())[item.getItemMeta().getPersistentDataContainer()
									.get(templeKey, PersistentDataType.INTEGER)],
							temples.get((temples.keySet().toArray())[item.getItemMeta().getPersistentDataContainer()
									.get(templeKey, PersistentDataType.INTEGER)]));
				else
					printGodDistance(player,
							isSummon(temples.get((temples.keySet().toArray())[item.getItemMeta()
									.getPersistentDataContainer().get(templeKey, PersistentDataType.INTEGER)])),
							temples.get((temples.keySet().toArray())[item.getItemMeta().getPersistentDataContainer()
									.get(templeKey, PersistentDataType.INTEGER)]).getGod());
			}
	...
	private void printStructureDistance(Player player, BoundingBox box, Structure structure) {
		...
		BaseComponent txt = new TextComponent("§1" + distance + GameSettings.LANG.textOf("event.divineTrackerTrack")
				+ structure.getGod().getName() + " temple");
		player.spigot().sendMessage(ChatMessageType.ACTION_BAR, txt);
	}
	...
	private void printGodDistance(Player player, Player target, GodsEnum god) {
		...
		BaseComponent txt = new TextComponent(
