...
public class PlayerBreakBlockListener implements Listener {
  ...
  public void onBreak(BlockBreakEvent e) {
    ...
    try {
      ...
      switch (block) {
        ...
        case DARK_OAK_WOOD:
          {
            if (GameSettings.CUT_CLEAN) {
              e.getBlock()
                  .getWorld()
                  .dropItemNaturally(e.getBlock().getLocation(), block.getItemStack());
        ...
        case BIRCH_LEAVES:
          dropItem(
              e.getBlock().getLocation(), block.getItemStack(), GameSettings.APPLE_DROP * 0.01);
          break;
      ...
      if (block.getMaterial() != Material.ANCIENT_DEBRIS
          && block.getMaterial() != Material.BIRCH_LEAVES) {
        Bukkit.getScheduler()
            .scheduleSyncDelayedTask(
                Daedalus.getInstance(), () -> replaceBlock(state), block.getSeconds() * 20L);
      }
              ...
              .runTaskTimer(
                  ...
                  () -> {
                    Collection<Entity> nearbyEntities =
                        state.getWorld().getNearbyEntities(state.getLocation(), 1, 1, 1);
  ...
  private void dropItem(Location loc, ItemStack itemStack, double percent) {
    ...
    if (new Random().nextDouble() <= percent) loc.getWorld().dropItemNaturally(loc, itemStack);
  }
