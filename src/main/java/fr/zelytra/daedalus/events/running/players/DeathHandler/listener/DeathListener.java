...
public class DeathListener implements Listener {
  ...
  public void onCustomDeath(EntityDamageEvent e) {
    ...
    if (daedalus.getGameManager().isRunning() && isDead) {
      ...
      boolean shrinkHasReachSpawn =
          daedalus.getStructureManager().getShrinkManager().getBorderRadius() <= 495;
      ...
      if (e instanceof EntityDamageByEntityEvent) {
        ...
        if (damageByEntityEvent.getDamager() instanceof Player) { // Kill by a player
          ...
          killByAMino =
              daedalus.getGameManager().getFactionManager().getFactionOf(killer).getGodsEnum()
                  == GodsEnum.MINOTAURE; // Tuer par un mino

        } else if (projectileSource != null
            && projectileSource instanceof Player) { // Kill by projectil
      ...
      boolean isMemberOfMino =
          daedalus.getGameManager().getFactionManager().getFactionOf(player).getGodsEnum()
              == GodsEnum.MINOTAURE; // Quand on est un mino
      ...
      if (hasMinoSpawn
          ...
          && daedalus
                  .getGameManager()
                  .getFactionManager()
                  .getFactionOf(FactionsEnum.MINOTAUR)
                  .getGod()
              != null) {
  ...
  public void onMinoSpawn(GodSpawnEvent e) {
    if (e.getGod() == GodsEnum.MINOTAURE) hasMinoSpawn = true;
  }
  ...
  private void winFX(Faction team) {
    for (Player p : Bukkit.getOnlinePlayers()) {
      p.sendTitle(
          "§l"
              + team.getType().getChatColor()
              + team.getType().getName()
              + GameSettings.LANG.textOf("event.victoryTitle"),
          GameSettings.LANG.textOf("event.victorySubTitle"),
          5,
          100,
          5);
    }
